...
public interface AdminMapper extends CommonMapper {
    ...
    @Insert("insert into doctor(doctor_no, doctor_name, phone, doctor_pic, department, title)" +
            " VALUES (#{doctorNo},#{doctorName},#{phone},#{pic},#{department},#{title})")
    int insertDoctor(AdminRegisterDoctorDTO dto);   // 返回受影响的行数


    // 插入化验员信息，service层已做校验，故而这里不需要插入role，直接数据库默认”化验员“
    @Insert("insert into lab_tech(lab_no, lab_name, phone, lab_pic) values(#{labNo},#{labName},#{phone},#{pic})")
